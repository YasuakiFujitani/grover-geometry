...
class UndirectedEdgeGraph():
    ...
    def compute_ricci_curvature_between_edge_undirected(self):
        ...
        for pair in pairs:
            ...
            cost_matrix_between_neighbors = [[self.distance_between_edges(snbr,tnbr) for snbr in source_nbr] for tnbr in target_nbr]
            output[(source,target)] = 1 - ot.emd2(np.array(source_nbr_distribution), np.array(target_nbr_distribution), cost_matrix_between_neighbors) / self.distance_between_edges(source, target)
        return output
