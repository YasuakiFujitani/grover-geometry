...
def compute_ricci_curvature_between_nodes_directed(G : nx.DiGraph, alpha):
    ...
    for pair in pairs:
        ...
        cost_matrix_between_neighbors = [[distance_between_nodes(snbr,tnbr,G) for snbr in source_nbr] for tnbr in target_nbr]
        output[(source,target)] = (1 - ot.emd2(np.array(source_nbr_distribution), np.array(target_nbr_distribution), cost_matrix_between_neighbors) / distance_between_nodes(source, target,G)) / (1-alpha)
    return output
